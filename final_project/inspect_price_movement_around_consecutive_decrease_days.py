...
'''Plot the stock price trend around the instances where a consecutive 5 days of decreases are observed, 
in an attempt to get insights'''
def plot_prices_around_decrease_day(symbol, n, pre_days, post_days, start_date, end_date, i):
    ...
    if i < len(decrease_days):
        ...
        if target_day in plot_data.index:
            plt.scatter(target_day, plot_data.loc[target_day, 'Close'], color='red', zorder=5)
...
pre_days = 15  # Days to plot before the decrease day
post_days = 15  # Days to plot after the decrease day
start_date = '2010-01-01'  # Start date for the data
end_date = '2014-01-01'  # End date for the data
i = 5 # instance number of the decreasing pattern
